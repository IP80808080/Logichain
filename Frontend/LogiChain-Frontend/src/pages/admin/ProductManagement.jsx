...
function ProductManagement() {
  ...
  useEffect(() => {
    fetchData(filterProductManager);
    ...
    try {
      ...
      const productsPromise =
        managerId === "All"
          ...
          : productService.getManagerProducts(managerId);

      const [productsRes, inventoryRes, usersRes] = await Promise.all([
        productsPromise,
  ...
  /**
   * FIXED: Get product stock by matching productId directly
   * This matches the SQL query logic where inventory.product_id = product.id
   */
  const getProductStock = (product) => {
    if (!product || !inventory.length) return 0;

    // Filter inventory by productId and sum quantities
    const total = inventory
      .filter((inv) => {
        // Match by productId (this is the key field that links inventory to product)
        return Number(inv.productId) === Number(product.id);
      })
      .reduce((sum, inv) => {
  ...
  /**
   * FIXED: Get reserved quantity for a product
   */
  const getProductReservedStock = (product) => {
    ...
    const totalReserved = inventory
      ...
      .reduce((sum, inv) => {
        return sum + (Number(inv.reservedQuantity) || 0);
      }, 0);
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      await fetchData(filterProductManager);
  ...
  const handleRestockSubmit = async () => {
    try {
      ...
      // Here you would typically call an API to create a restock request
      // await restockService.create({
      //   productId: restockingProduct.id,
      //   quantity: restockData.quantity,
      //   message: restockData.message,
      //   createdBy: restockingProduct.createdBy
      // });
  ...
  // FIXED: Calculate products with stock using the corrected function
  ...
  // FIXED: Calculate total metrics correctly
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4 pt-4">
                  <button
                    ...
                    className="flex-1 px-4 py-2 bg-gray-200 text-white rounded-xl hover:bg-gray-300 transition"
                    disabled={loading}
                  >
                    Cancel
                  </button>
