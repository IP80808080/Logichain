...
public class UserServiceImpl implements IUserService {
    ...
    public UserDTO createUser(CreateUserRequest request) {
        ...
        if (user.getRole() == Role.WAREHOUSE_MANAGER || 
                user.getRole() == Role.CUSTOMER_SUPPORT || 
    ...
    public void delete(Long id) {
        ...
        // 1. Check if the user has any linked orders (Foreign Key check)
        // Note: You need to inject your OrderRepository or use the Query above in UserRepository
        boolean hasLinkedRecords = repo.hasOrders(id); 

        if (hasLinkedRecords) {
            // SOFT DELETE: The user has orders, so we just deactivate them
            user.setApprovalStatus(ApprovalStatus.REJECTED);
            user.setRejectionReason("Account deactivated: User has existing order history.");
            user.setUpdatedAt(LocalDateTime.now());
            repo.save(user);
        } else {
            // HARD DELETE: No orders found, safe to remove from database
            repo.delete(user);
        }
    }
